/*
  ...
  Input: { m, n, Mat[m x n] }, where:
    ...
    c = mat[0,1..n]f .. row 0 is z to maximize, note c is negated in input.
...
void read_tableau(Tableau *tab, const char * filename) {
    ...
    memset(tab, 0, sizeof(*tab)); //C standard library - intiallise
    //first argument - tabobject in the memmory, byte variable, third argument spcifies whats get written
    err = fscanf(fp, "%d %d", &tab->m, &tab->n);//fscanf - takes a line from the file pointer and puts them in to the feilds of the tabl
    if (err == 0 || err == EOF) {
...
void pivot_on(Tableau *tab, int row, int col) {
    ...
    assert(pivot>0); // if the expression false the program terminates
    for(j=0;j<tab->n;j++)
        tab->mat[row][j] /= pivot;
    ...
    for(i=0; i<tab->m; i++) { // foreach remaining row i do
        ...
        if(i==row) continue; // if it the current row skip the iteration and got to the next
        for(j=0; j<tab->n; j++) { // r[i] = r[i] - z * r[row];
            tab->mat[i][j] -= multiplier * tab->mat[row][j]; //subtracts multiplier from the value of the current matrix
        }
    }
}
...
void check_b_positive(Tableau *tab) {
    int i;
    for(i=1; i<tab->m; i++)
        assert(tab->mat[i][0] >= 0); //Assert - error checking
}
...
void simplex(Tableau *tab) {
    ...
    while( ++loop ) { //++ prefix operater for every iteration increases the loop
        int pivot_col, pivot_row;

        pivot_col = find_pivot_column(tab);
        if( pivot_col < 0 ) { // no negative values found
        ...
        if (pivot_row < 0) { // if no pivout row breaks from the loop
